...
## ⚡ Session Bootstrap (RUNS FIRST — EVERY SESSION)

At the start of EVERY session, before responding to any message:

1. **Read `.claude/MASTER_PROMPT.md`** in the current project directory
   - If not found, read `~/.claude/MASTER_PROMPT.md` (global fallback)
   - Apply all operating principles from it immediately
...
## Agent Delegation
- Full roster: `.claude/AGENTS.md` (auto-generated by `/metaprompt` per project)
...
## Agent Roster
See `.claude/AGENTS.md` for full project-specific definitions (auto-created by `/metaprompt`).
Universal agents present in every project:

| Agent | Trigger | Domain |
|-------|---------|--------|
| `code-reviewer` | Before any ship | All source files — blocks on secrets, injection, memory leaks |
| `doc-writer` | After any T2/T3 | README, inline docs, CHANGELOG |
| Project specialists | Detected by `/metaprompt` | Stack-specific (see AGENTS.md) |
